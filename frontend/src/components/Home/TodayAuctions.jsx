...
const TodayAuctions = () => {
  ...
  const todayAuctions = allAuctions.filter(
    (auction) => new Date(auction.startTime).toDateString() === todayDate
  );
      ...
      <section className="my-8">
        ...
        <div className="flex flex-wrap gap-6">
          ...
          <div className="flex flex-col gap-4 w-full lg:flex-1 2xl:flex-none 2xl:basis-64 2xl:flex-grow">
            {todayAuctions.slice(0, 2).map((element) => {
              return (
                <Link
          ...
          <div className="flex flex-col gap-4 w-full 2xl:basis-64 2xl:flex-grow">
            {todayAuctions.slice(2, 4).map((element) => {
              return (
                <Link
                  to={`/auction/item/${element._id}`}
                  key={element._id}
          ...
          {/* #3 */}
          <div className="flex flex-col gap-4 w-full 2xl:basis-64 2xl:flex-grow">
            {todayAuctions.slice(4, 6).map((element) => {
              return (
                <Link
                  to={`/auction/item/${element._id}`}
                  key={element._id}
