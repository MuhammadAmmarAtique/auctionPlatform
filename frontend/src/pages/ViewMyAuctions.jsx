...
import { getUserAuctionItems } from "../store/slices/auctionSlice";
  ...
  return (
    <>
      <div className="w-full ml-0 m-0 h-fit px-5 pt-20 lg:pl-[320px] flex flex-col">
        <h1
          className={`text-[#d6482b] text-2xl font-bold mb-5 min-[480px]:text-4xl md:text-6xl xl:text-7xl 2xl:text-8xl`}
        >
          My Auctions
        </h1>
        {loading ? (
          ...
          <div
            className={`${
              userAuctions.length > 2 && "flex-grow"
            } flex flex-wrap gap-6`}
          >
            {userAuctions.length > 0 ? (
              userAuctions.map((element) => {
                return (
                  <CardTwo
                    title={element.title}
